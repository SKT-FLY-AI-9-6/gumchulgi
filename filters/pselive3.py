...
class LiveFilter3:
    ...
    def _mask(self, bgr_small):
        ...
        # **순 방향성 관문** — 규격의 "1/4 이상 동시에 플래시"는 그 면적이 같은
        # 방향으로 변했다는 뜻이다. 팬은 한쪽에서 밝은 것이 들어오고 반대쪽으로
        # 나가므로 up 과 dn 면적이 맞먹어 상쇄된다. 진짜 점멸은 한 방향이 압도한다.
...
def run(src, cfg: Cfg = None, video_out=None, lossless=False, verbose=True):
    ...
    rep = {"src": src, "frames": n, "fps": round(fps, 3), "duration_s": round(dur, 2),
           ...
           "net_blocked": live.stats["net_blocked"],
