...
class TPeakValley:
    ...
    def step(self, X, theta_dark, michelson):
        if not self.ring:
            ...
            self.last_dn = torch.zeros_like(X, dtype=torch.bool)
        ...
        if len(self.ring) > self.n:
            self.ring.pop(0)
        # 순 방향성 관문이 하강분을 쓴다. up 과 down 은 both 해소 뒤 서로소다.
        self.last_dn = down
        return up
...
def translate_gpu(x, dx, dy, gate, zeros=False):
    ...
    return F.grid_sample(x, grid, mode="bilinear",
                         padding_mode=("zeros" if zeros else "border"),
                         align_corners=True)
...
class FullFilterGPU:
    ...
    def _detect(self, lin_s):
        ...
        # 순 방향성 관문 — 팬은 up/dn 면적이 맞먹어 상쇄되고 점멸만 남는다.
        # CPU 기준판과 같은 **전역 스칼라** 비교로 맞춘다 (area_wcag 는 창 최대값).
    ...
    def _step(self):
        ...
        if c.motion_comp and self.prev_gray is not None:
            ...
            sc0 = self.W / float(self.aw)
            mok = ((ratio > self.o.peak_ratio_min)
                   & (mdx.abs() * sc0 < c.motion_max_px)
                   & (mdy.abs() * sc0 < c.motion_max_px)
                   & ((mdx.abs() > 0.5) | (mdy.abs() > 0.5))).to(torch.float32)
...
def run(src, cfg: P3.Cfg = None, opt: OptF = None, video_out=None,
    ...
    rep = {"src": src, "res": f"{W}x{H}", "frames": n, "dtype": str(live.dt),
           ...
           "net_blocked": int(live.stats["net_blocked"]),
