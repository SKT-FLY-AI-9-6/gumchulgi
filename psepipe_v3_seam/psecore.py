# -*- coding: utf-8 -*-
"""
psecore.py — 통합 광과민성 검출기 v2.0  [정본 아님 — 2026-08-12]
================================================================================
** 규격 판정 정본은 pse_bt1702.py 다. ** 18클립 실측에서 psecore 는 ③패턴·
⑤화면전환 채널이 없어 v_stripes / v_cuts6 위반을 통과시켰다 (14/17 vs 16/17).
파이프라인(psepipe)·psefield 는 이 모듈을 더 이상 쓰지 않는다.
이 파일은 **규격 밖 임상 채널(채널별 RGB, 적청 RB) 단독 분석 도구**로만 남긴다 —
pse_bt1702 에 없는 채널별 RGB excursion 분석이 필요할 때만 쓸 것.
================================================================================
...
def global_shift(prev_gray: np.ndarray, curr_gray: np.ndarray,
                 min_resp: float = 0.10, max_px: float = 0.25):
    """전역 이동 추정. **신뢰도가 낮으면 0 을 반환한다.**

    실측 버그 — 주기 패턴(줄무늬)에서 위상상관은 다봉이라 아무 봉우리나 잡는다.
    14번 클립(흐르는 줄무늬 10쌍)에서 응답이 0.003~0.05 인데도 ±190px 씩 워프했고,
    그 워프가 만든 어긋남이 **없던 위반 1.26초**로 잡혔다.
    움직임 보상을 끄면 같은 클립이 0.00s PASS 다. 즉 검출기 자신의 허위 양성이었다.

    그래서 (a) 응답 하한과 (b) 프레임 크기 대비 이동 상한을 함께 건다.
    실사(seg6)에서는 보상이 여전히 도움이 된다(6.34s vs 무보상 7.00s).
    """
    try:
        (dx, dy), resp = cv2.phaseCorrelate(prev_gray.astype(np.float32),
                                            curr_gray.astype(np.float32))
    except cv2.error:
        return 0.0, 0.0
    h, w = curr_gray.shape[:2]
    if resp < min_resp or abs(dx) > max_px * w or abs(dy) > max_px * h:
        return 0.0, 0.0
...
def analyze(path, cfg: Cfg = None, profile_name: str = "custom",
            want_masks: bool = False, progress=None, fps: float = None,
            src_hw=None) -> Report:
    """
    ...
    **path 대신 프레임 이터러블(BGR uint8)을 줄 수도 있다.** 그 경우 fps 가
    필수다. 필터의 폐루프 검증에서 쓴다 — 라운드마다 FFV1 로 썼다가 다시
    디코드하던 왕복이 전체 시간의 대부분이었다(실측: 판정 70% + 인코딩 18%).
    프레임을 그대로 넘기면 무손실보다 더 정확하고(인코딩 자체가 없다) 훨씬 빠르다.
    축소는 여기서 path 경로와 **똑같이** INTER_AREA 로 하므로 판정이 일치한다.
    """
    ...
    if isinstance(path, (str, os.PathLike)):
        ...
        # **제너레이터를 리스트로 만들지 않는다.** 1080p 4분이면 37GB 다.
        it = iter(path)
        try:
            first = next(it)
        ...
        h, w = first.shape[:2]
        sh, sw = src_hw or (h, w)
        if min(w, h) <= cfg.short_side:
            aw, ah = w, h
        else:
            sc = cfg.short_side / min(w, h)
            aw, ah = max(2, int(round(w * sc))), max(2, int(round(h * sc)))
        n_decl = len(path) if hasattr(path, "__len__") else 0
        cap, info = None, VideoInfo("<frames>", float(fps), n_decl, sw, sh, aw, ah)
    ...
    def _feed():
        if cap is not None:
            yield from iter_frames(cap, info)
            return
        for f in src:
            yield (cv2.resize(f, (info.ana_w, info.ana_h),
                              interpolation=cv2.INTER_AREA)
                   if f.shape[1::-1] != (info.ana_w, info.ana_h) else f)

    try:
        for idx, bgr in enumerate(_feed()):
            t_ms = idx * frame_ms
            n = idx + 1
    ...
    rep = Report(
        source=str(path) if cap is not None else "<frames>", profile=profile_name,
