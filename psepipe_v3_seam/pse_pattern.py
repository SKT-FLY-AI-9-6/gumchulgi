...
def _finish(series, masks, fps, video):
    ...
    # **가장 위반에 가까웠던 프레임**의 축 값 — 축을 따로 최대화하면
    # 서로 다른 프레임의 값이 섞여 여백이 실제보다 없어 보인다.
    # (실측: whip_pan 이 쌍수 224% / 면적 235% 로 보였는데, 같은 프레임에서
    #  재면 두 축이 동시에 높은 프레임이 없어 '안전' 이 된다.)
    best, bi = -1.0, None
    for i, s in enumerate(series):
        mv = bool(moving[i])
        np_, na = (PAIRS_MIN_DRIFT if mv else PAIRS_MIN_STATIC), (AREA_DYNAMIC if mv else AREA_STATIC)
        v = min(s["pairs"] / np_, s["area"] / na)
        if v > best:
            best, bi = v, i
    co = {"pairs": series[bi]["pairs"], "area_pct": round(series[bi]["area"]*100, 1),
          "need_pairs": PAIRS_MIN_DRIFT if moving[bi] else PAIRS_MIN_STATIC,
    ...
    return {"co_axes": co,
            ...
            "segments": segs, "_series": series, "_masks": masks}
