...
1. ~~**적색 선형광 정의 확인** (①)~~ → **해결** (아래 "추가 기록" — 선형광이
   맞고, 패치 적용, 합성 24편 IRIS 일치 20/24 + 실영상 7편 판정 불변 확인).
...
## 추가 기록 (2026-08-13, 남은 것 1 해결)

**적색 선형광 정의 — 원문 확인 완료, 패치 적용됨.**

- IRIS 원문 실측: `config/appsettings.json` 의 `sRGBValues` LUT 첫 값이
  0.000303527 = (1/255)/12.92 — 정확히 sRGB EOTF 복원표다. 프레임이 이 표로
  선형광(0~1)으로 변환된 뒤 `RedSaturation` 이 R/(R+G+B) 를 계산한다. 확정.
- WCAG 2.2 원문(w3c/wcag `general-flash-and-red-flash-thresholds`): 적색 플래시
  = "R/(R+G+B) ≥ 0.8 인 상태로/에서의 전환 + **CIE 1976 UCS 색도도에서 0.2
  초과 차이**". u'v' 는 선형광 삼자극치에서만 정의되므로, 같은 문장의 채도비만
  감마 값으로 읽는 해석이 오히려 비정합 — 선형광 해석이 문언에 충실하다.
  (우리 검출기도 이미 Δu'v' 는 선형광으로 계산하고 있었다 — 내부 비일관이었던 것.)
- 패치: `pse_bt1702.saturated_red()` 를 선형광 기준으로 변경 (`fix` 브랜치).
  A/B 결과 — 합성 24편: **07·08 만 위반—적색으로 뒤집혀 IRIS 와 일치 18→20/24**
  (잔여 4건은 컷 축 부재 ②·시퀀스 규칙 ③·화소 동일성 ④ 의 규격 계열 차이로
  전부 기해명). 실영상 7편(위반 4·적합 3, cera 적색 조명 포함): **판정 변화 0**.
  선형 정의로 채도적색 마스크 면적은 커지지만(예: Db4Sor9OcyL 최대 8.9→49.8%)
  전이 조건(Δu'v' ≥ 0.2 + 면적 25% + 3회/s)이 그대로 걸러낸다. 남은 확인은
  팀 19편 전체 + 붉은 실사 표본으로의 확대 재실행 정도.
