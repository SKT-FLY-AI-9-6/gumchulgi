...
class FeedController extends AsyncNotifier<List<FeedVideo>> {
  ...
  Future<List<FeedVideo>> build() async {
    _cursor = null;
    _end = false;
    final (vids, next) = await ref.read(apiProvider).feed(limit: _feedPageSize);
    _cursor = next;
    _end = next == null || vids.isEmpty;
    return vids;
  }
  ...
  Future<void> loadMore() async {
    ...
    _loading = true;
    try {
      final (vids, next) = await ref
          .read(apiProvider)
          .feed(cursor: _cursor, limit: _feedPageSize);
      _cursor = next;
      _end = next == null || vids.isEmpty;
...
final feedProvider = AsyncNotifierProvider<FeedController, List<FeedVideo>>(
  FeedController.new,
);
