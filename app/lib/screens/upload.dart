...
class _UploadState extends ConsumerState<UploadScreen> {
  ...
  Future<void> _pick() async {
    ...
    setState(() {
      ...
      _fileBytes = bytes;
      _phase = _Phase.idle;
      _failureMessage = bytes > maxUploadBytes
          ? '파일 크기가 ${_formatBytes(bytes)}입니다. 최대 ${maxUploadMb}MB까지 업로드할 수 있습니다.'
          : null;
    });
  }
  ...
  Future<void> _upload() async {
    ...
    final bytes = _fileBytes ?? await _file!.length();
    if (bytes > maxUploadBytes) {
      setState(() => _failureMessage = '최대 ${maxUploadMb}MB까지 업로드할 수 있습니다.');
      return;
    }
    setState(() {
    ...
    try {
      ...
      setState(() {
        ...
        _failureMessage = _uploadError(e);
      });
    }
  }
  ...
  String get _sizeLabel {
    return _formatBytes(_fileBytes ?? 0);
  }

  String _uploadError(Object error) {
    if (error is ArgumentError) {
      return error.message?.toString() ?? '최대 ${maxUploadMb}MB까지 업로드할 수 있습니다.';
    }
    if (error is DioException) {
      ...
      if (error.type == DioExceptionType.connectionTimeout ||
        ...
        return '업로드 시간이 초과됐습니다. 네트워크 상태를 확인하고 다시 시도해주세요.';
      }
      if (error.response == null) {
        return '서버에 연결하지 못했습니다. 네트워크 연결을 확인해주세요.';
      }
    }
    return '업로드에 실패했습니다. 파일 형식과 길이 제한을 확인해주세요.';
  }
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              color: run
                  ? null
                  : done
                  ? Aurora.teal.withValues(alpha: .16)
                  : Aurora.ink2,
              border: run
                  ? null
                  : Border.all(
                      color: done
                          ? Aurora.teal.withValues(alpha: .5)
                          : Aurora.line,
                    ),
            ),
            child: done
        ...
        child: ListView(
          ...
          children: [
                ...
                child: Container(
                  ...
                  child: const Column(
                    ...
                    children: [
                      ...
                      Text(
                        '최대 500MB · 3분',
                        style: TextStyle(fontSize: 10.5, color: Aurora.sub),
                      ),
                    ],
                  ),
                ),
            ...
            if (_phase == _Phase.idle) ...[
              ...
              AuroraButton(
                label: '안전 처리 시작',
                onTap: _file == null || _failureMessage != null
                    ? null
                    : _upload,
              ),
            ],
                  ...
                  children: [
                    ...
                    Text(
                      switch (_phase) {
                        ...
                        _ =>
                          _failureMessage ??
                              '크기(${maxUploadMb}MB)·길이(3분) 제한을 확인하고 다시 시도해주세요.',
                      },
                      style: const TextStyle(
                        fontSize: 11.5,
                        color: Aurora.sub,
    ...
    child: Row(
      children: [
        Expanded(
          child: Text(
            l,
            style: const TextStyle(fontSize: 12.5, color: Aurora.sub),
          ),
        ),
        Text(
          r,
