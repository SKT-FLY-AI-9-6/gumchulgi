...
class _LoginState extends ConsumerState<LoginScreen> {
  ...
  String _errorText(Object? e) {
    if (e is DioException) {
      ...
      if (code == 401) return '이메일 또는 비밀번호가 틀렸습니다';
      if (code == 422) return '입력을 확인하세요 (비밀번호 8자 이상, 이메일 형식)';
      if (code != null && code >= 500) {
        return '서버가 잠시 응답하지 않습니다. 잠시 후 다시 시도해주세요';
      }
      if (e.type == DioExceptionType.connectionTimeout ||
          e.type == DioExceptionType.sendTimeout ||
          e.type == DioExceptionType.receiveTimeout) {
        return '로그인 시간이 초과됐습니다. 네트워크 상태를 확인해주세요';
      }
      if (e.response == null) return '서버에 연결할 수 없습니다. 네트워크 상태를 확인해주세요';
    }
  ...
  void _submit() {
    ...
    } else if (_signup && nickname.isEmpty) {
      error = '닉네임을 입력해주세요';
    }
    ...
    _signup
        ? notifier.signup(email, password, nickname)
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: auth.isLoading
                          ? null
                          : () {
                              ref.read(authProvider.notifier).clearFailure();
                              setState(() {
                                _signup = !_signup;
